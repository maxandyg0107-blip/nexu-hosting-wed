<?php
/**
 * NEXU HOSTING - Registro de Cuenta
 */
require_once __DIR__ . '/config/bootstrap.php';

if (isLoggedIn()) redirect('/dashboard.php');

$page_title       = 'Crear cuenta';
$meta_description = 'Regístrate gratis en Nexu Hosting y activa tu servidor en minutos.';
?>
<?php require_once __DIR__ . '/views/partials/head.php'; ?>

<div class="min-h-screen flex flex-col">
  <?php require_once __DIR__ . '/views/partials/navbar.php'; ?>

  <main class="flex-1 flex items-center justify-center px-4 py-24">
    <div class="w-full max-w-md animate-fade-in">

      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-600 to-cyan-500 shadow-lg shadow-brand-600/40 mb-4">
          <span class="text-white font-black text-xl">N</span>
        </div>
        <h1 class="text-2xl font-extrabold text-white">Crea tu cuenta</h1>
        <p class="text-gray-400 mt-1 text-sm">Únete a miles de gamers en Nexu Hosting</p>
      </div>

      <div data-flash><?= renderFlash() ?></div>

      <div class="bg-surface-card border border-surface-border rounded-2xl p-8 shadow-2xl shadow-black/40">

        <!-- OAuth -->
        <div class="grid grid-cols-2 gap-3 mb-6">
          <a href="/auth/google/redirect.php"
             class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-surface-border bg-surface hover:bg-surface-hover hover:border-white/20 transition-all text-sm font-medium text-gray-300">
            <svg class="w-4 h-4" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
            Google
          </a>
          <a href="/auth/discord/redirect.php"
             class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-[#5865F2]/30 bg-[#5865F2]/10 hover:bg-[#5865F2]/20 transition-all text-sm font-medium text-[#7289DA]">
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z"/></svg>
            Discord
          </a>
        </div>

        <div class="flex items-center gap-4 mb-6">
          <div class="flex-1 h-px bg-surface-border"></div>
          <span class="text-xs text-gray-500 uppercase tracking-wider">o con email</span>
          <div class="flex-1 h-px bg-surface-border"></div>
        </div>

        <form method="POST" action="/auth/register.php" class="space-y-4">
          <?= csrfField() ?>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1.5">Usuario <span class="text-red-400">*</span></label>
              <input type="text" name="username" required minlength="3" maxlength="50"
                     pattern="[a-zA-Z0-9_.-]+" title="Solo letras, números, puntos y guiones"
                     value="<?= e($_POST['username'] ?? '') ?>"
                     class="w-full px-3 py-2.5 rounded-xl bg-surface border border-surface-border text-white placeholder-gray-500 text-sm focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                     placeholder="gamer123">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1.5">Nombre completo</label>
              <input type="text" name="full_name"
                     value="<?= e($_POST['full_name'] ?? '') ?>"
                     class="w-full px-3 py-2.5 rounded-xl bg-surface border border-surface-border text-white placeholder-gray-500 text-sm focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                     placeholder="Juan Pérez">
            </div>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Correo electrónico <span class="text-red-400">*</span></label>
            <input type="email" name="email" required autocomplete="email"
                   value="<?= e($_POST['email'] ?? '') ?>"
                   class="w-full px-4 py-2.5 rounded-xl bg-surface border border-surface-border text-white placeholder-gray-500 text-sm focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                   placeholder="tu@email.com">
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1.5">Contraseña <span class="text-red-400">*</span></label>
              <input type="password" name="password" required minlength="8" autocomplete="new-password"
                     class="w-full px-3 py-2.5 rounded-xl bg-surface border border-surface-border text-white placeholder-gray-500 text-sm focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                     placeholder="••••••••">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1.5">Confirmar <span class="text-red-400">*</span></label>
              <input type="password" name="password2" required minlength="8"
                     class="w-full px-3 py-2.5 rounded-xl bg-surface border border-surface-border text-white placeholder-gray-500 text-sm focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                     placeholder="••••••••">
            </div>
          </div>

          <p class="text-xs text-gray-500">
            Al registrarte aceptas nuestros
            <a href="/terminos.php" class="text-brand-400 hover:underline">Términos de Servicio</a> y
            <a href="/privacidad.php" class="text-brand-400 hover:underline">Política de Privacidad</a>.
          </p>

          <button type="submit"
                  class="w-full py-2.5 rounded-xl bg-gradient-to-r from-brand-600 to-brand-500 text-white font-semibold text-sm
                         shadow-lg shadow-brand-600/30 hover:shadow-brand-600/50 hover:-translate-y-px active:translate-y-0
                         transition-all duration-150">
            Crear cuenta gratis
          </button>
        </form>

        <p class="text-center text-sm text-gray-500 mt-6">
          ¿Ya tienes cuenta?
          <a href="/login.php" class="text-brand-400 hover:text-brand-300 font-medium transition-colors">Inicia sesión</a>
        </p>
      </div>
    </div>
  </main>

  <?php require_once __DIR__ . '/views/partials/footer.php'; ?>
</div>
