<?php
/**
 * NEXU HOSTING - Página de Inicio de Sesión
 * Vista: Login local + OAuth Google / Discord + CSRF + hardening
 */
declare(strict_types=1);

require_once __DIR__ . '/config/bootstrap.php';

// ─── Protección de sesión: si ya está autenticado, redirigir inmediatamente ───
if (isLoggedIn()) {
    redirect(isAdmin() ? '/admin/dashboard.php' : '/dashboard.php');
    exit; // Doble seguridad: cortar ejecución
}

$page_title       = 'Iniciar sesión';
$meta_description = 'Accede a tu panel de cliente de Nexu Hosting.';

// Sanitizar email pre-llenado tras error de autenticación (evita XSS y notices)
$prefillEmail = '';
  ...
  <main class="flex-1 flex items-center justify-center px-4 py-24">
    <div class="w-full max-w-md animate-fade-in">
      ...
      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-600 to-cyan-500 shadow-lg shadow-brand-600/40 mb-4" aria-hidden="true">
          <span class="text-white font-black text-xl">N</span>
        </div>
        <h1 class="text-2xl font-extrabold text-white">Bienvenido de nuevo</h1>
        <p class="text-gray-400 mt-1 text-sm">Ingresa a tu cuenta de Nexu Hosting</p>
      </div>
      ...
      <div class="bg-surface-card border border-surface-border rounded-2xl p-8 shadow-2xl shadow-black/40">
        ...
        <div class="grid grid-cols-2 gap-3 mb-6">
          <a href="/auth/google/redirect.php"
             class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-surface-border bg-surface hover:bg-surface-hover hover:border-white/20 transition-all text-sm font-medium text-gray-300"
          ...
          <a href="/auth/discord/redirect.php"
             class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-[#5865F2]/30 bg-[#5865F2]/10 hover:bg-[#5865F2]/20 transition-all text-sm font-medium text-[#7289DA]"
             rel="noopener noreferrer">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <span class="text-xs text-gray-500 uppercase tracking-wider">o con email</span>
          <div class="flex-1 h-px bg-surface-border"></div>
        </div>

        <!-- Login form -->
        <form method="POST" action="/auth/login.php" class="space-y-4" novalidate autocomplete="off">
          <?= csrfField() ?>

          <!-- Honeypot: campo invisible para bots. Debe permanecer vacío. -->
          ...
          <div>
            ...
            <input type="email" id="email" name="email" required autocomplete="email" inputmode="email" spellcheck="false"
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              ...
              <a href="/recuperar.php" class="text-xs text-brand-400 hover:text-brand-300 transition-colors">¿Olvidaste tu contraseña?</a>
            </div>
          ...
          <button type="submit"
                  class="w-full py-2.5 rounded-xl bg-gradient-to-r from-brand-600 to-brand-500 text-white font-semibold text-sm
                         shadow-lg shadow-brand-600/30 hover:shadow-brand-600/50 hover:-translate-y-px active:translate-y-0
        ...
        <p class="text-center text-sm text-gray-500 mt-6">
          ¿No tienes cuenta?
          <a href="/register.php" class="text-brand-400 hover:text-brand-300 font-medium transition-colors">Créala gratis</a>
        </p>
      </div>
    </div>
  </main>
